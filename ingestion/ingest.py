...
class DocumentIngestionPipeline:
    ...
    def __init__(
        ...
        export_markdown_dir: Optional[str] = None,
    ):
        """
        ...
        Args:
            ...
            export_markdown_dir: Optional output directory to save converted markdown files
        """
        self.config = config
        self.documents_folder = documents_folder
        self.clean_before_ingest = clean_before_ingest
        self.export_markdown_dir = export_markdown_dir
    ...
    async def initialize(self):
        """Initialize database connections and check environment."""
        if self._initialized:
            return
        
        logger.info("Initializing ingestion pipeline...")
        
        # Check GPU availability for user awareness. This is done early to provide fast feedback.
        try:
            import torch
            if torch.cuda.is_available():
                logger.info(f"🚀 GPU detected: {torch.cuda.get_device_name(0)}")
            else:
                logger.warning("⚠️  No GPU detected. Ingestion will use CPU and may be slow.")
                logger.warning(f"   Debug: Torch version: {torch.__version__}, CUDA available: {torch.cuda.is_available()}")
                if torch.version.cuda is None:
                    logger.warning("   Torch appears to be a CPU-only build (torch.version.cuda=None).")
                    logger.warning("   Install CUDA-enabled torch into this project venv:")
                    logger.warning(
                        "   uv pip install --python .venv\\Scripts\\python.exe torch torchvision torchaudio "
                        "--index-url https://download.pytorch.org/whl/cu126 --reinstall"
                    )
                logger.warning("   For GPU acceleration, see the 'GPU Acceleration' section in README.md.")
        except ImportError:
            logger.info("PyTorch not found, GPU check skipped. GPU acceleration for document and audio processing will be unavailable.")

        # Initialize database connections
        await initialize_database()
        await self._validate_embedding_dimensions()
        
        # Verify embedding service connection (Fail fast if Ollama is down)
        if os.getenv("EMBEDDING_BACKEND", "openai") != "local":
            logger.info("Verifying connection to embedding service...")
            try:
                # Generate a tiny embedding to test connection
                await self.embedder.generate_embedding("test")
            except Exception as e:
                logger.error(f"❌ Could not connect to Embedding Service.")
                logger.error(f"   Please check if Ollama is running and OPENAI_BASE_URL is set correctly in .env")
                raise ConnectionError(f"Embedding service unreachable: {e}") from e
        
        self._initialized = True
        logger.info("Ingestion pipeline initialized")

    async def _validate_embedding_dimensions(self):
        """Ensure DB vector column dimensions match current embedding model output."""
        ...
        if not row or not row["embedding_type"]:
            logger.warning("Could not verify chunks.embedding dimensions; column metadata not found.")
            return

        match = re.match(r"vector\((\d+)\)", row["embedding_type"])
        if not match:
            logger.warning(f"Could not parse embedding type '{row['embedding_type']}' for dimension check.")
            return

        db_dim = int(match.group(1))
        if db_dim != expected_dim:
            raise ValueError(
                f"Embedding dimension mismatch: DB expects {db_dim}, model '{self.embedder.model}' outputs {expected_dim}. "
                "Update EMBEDDING_MODEL/EMBEDDING_DIMENSIONS or recreate schema with matching vector dimension."
            )
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        if self.export_markdown_dir:
            self._export_markdown_snapshot(file_path, document_content)
    ...
    def _export_markdown_snapshot(self, source_file_path: str, markdown_content: str):
        """Export normalized markdown content to disk for quality review."""
        rel_path = os.path.relpath(source_file_path, self.documents_folder)
        rel_no_ext = os.path.splitext(rel_path)[0]
        export_path = os.path.join(self.export_markdown_dir, f"{rel_no_ext}.md")
        ...
        logger.info(f"Exported markdown snapshot: {export_path}")
    ...
    def _get_docling_converter(self):
        """
        Get or create a configured Docling DocumentConverter.
        Uses lazy initialization to setup GPU acceleration only when needed.
        """
        ...
        try:
            ...
            logger.info("Initializing optimized Docling converter...")
            ...
            pipeline_options = ThreadedPdfPipelineOptions(
                ...
                table_batch_size=4,
                do_ocr=True  # Enable OCR for scanned docs (set False for speed if only using digital PDFs)
            )
            ...
            logger.warning(f"Could not initialize optimized converter ({e}), falling back to default.")
...
async def main():
    ...
    parser.add_argument(
        "--export-markdown-dir",
        default=None,
        help="Optional output directory to save converted markdown files before embedding",
    )
    ...
    # GPU availability is now checked within the DocumentIngestionPipeline.initialize() method,
    # which is called when the pipeline is run.
    ...
    pipeline = DocumentIngestionPipeline(
        ...
        export_markdown_dir=args.export_markdown_dir,
    )
