...
class EmbeddingGenerator:
    ...
    def __init__(
        ...
        if self.backend == "local":
            ...
            # Use a good default local model if standard OpenAI model name is passed
            local_model_name = "all-MiniLM-L6-v2" if "text-embedding" in model else model
            logger.info(f"Initializing local embedding model: {local_model_name}")
            self.local_model = SentenceTransformer(local_model_name)
            self.config = {"dimensions": self.local_model.get_sentence_embedding_dimension(), "max_tokens": 512}
        elif "nomic" in model.lower():
            logger.info(f"Detected Nomic model {model}, defaulting to 768 dimensions")
            self.config = {"dimensions": int(os.getenv("EMBEDDING_DIMENSIONS", "768")), "max_tokens": 8191}
        elif model not in self.model_configs:
            logger.warning(f"Unknown model {model}, using default config")
            # Allow overriding dimensions via env var, default to OpenAI's 1536
            self.config = {"dimensions": int(os.getenv("EMBEDDING_DIMENSIONS", "1536")), "max_tokens": 8191}
        else:
    ...
    async def generate_embedding(self, text: str) -> List[float]:
        ...
        if self.backend == "local":
            ...
            return await loop.run_in_executor(None, lambda: self.local_model.encode(text).tolist())
    ...
    async def generate_embeddings_batch(
        ...
        if self.backend == "local":
            # Run in executor to avoid blocking async loop
            loop = asyncio.get_running_loop()
            return await loop.run_in_executor(None, lambda: self.local_model.encode(processed_texts).tolist())
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # Check for connection errors specifically to give better hints
                if "Connection error" in str(e) and self.backend != "local":
                    logger.error(f"❌ Connection error connecting to Embedding API.")
                    if hasattr(embedding_client, 'base_url'):
                        logger.error(f"   Please check if the service at {embedding_client.base_url} is running.")
